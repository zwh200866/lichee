...
{
	printk(KERN_ALERT"~~~~~~~ my dbg, %s %d, start\n", __FUNCTION__,__LINE__);
	u8 data;
	int err;
	...
	dev->enabled = true;
	printk(KERN_ALERT"~~~~~~~ my dbg, %s %d, end\n", __FUNCTION__,__LINE__);
			      ...
			      char *buf)
{
	...
	mutex_unlock(&dev->lock);
	printk(KERN_ALERT"~~~~~~~ my dbg, %s %d, ,====%d\n", __FUNCTION__,__LINE__,val);
...
static ssize_t attr_get_temp(struct device *device,struct device_attribute *attr,char *buf){

	return sprintf(buf, "%d.%d\n",data_temp/1000,data_temp%1000);	
}
static ssize_t attr_get_hum(struct device *device,struct device_attribute *attr,char *buf){

	return sprintf(buf, "%d.%d\n",data_hum/1000,data_hum%1000);
}
static struct device_attribute hts221_sysfs_attributes[] = {
	...
	__ATTR(temp, 0444, attr_get_temp, NULL),
	__ATTR(hum, 0444, attr_get_hum, NULL),
};
...
static int hts221_get_data(struct hts221_dev *dev, int *data_t, int *data_h)
{
	...
	*data_h = hts221_convert(dev->sensors[HTS221_SENSOR_H].slope,
	...
	//printk(KERN_ALERT"-------------data_h=%d my dbg, %s %d\n",data_h, __FUNCTION__,__LINE__);
	...
	*data_t = hts221_convert(dev->sensors[HTS221_SENSOR_T].slope,
	...
	//printk(KERN_ALERT"-------------data_t=%d my dbg, %s %d\n",data_t, __FUNCTION__,__LINE__);
#ifdef HTS221_DEBUG
...
static void hts221_input_work_fn(struct work_struct *work)
{
	...
	printk(KERN_ALERT"-------------data_t=%d,data_h=%d   my dbg, %s %d\n",data_t,data_h,__FUNCTION__,__LINE__);
	data_temp = data_t;
	data_hum = data_h;
	mutex_unlock(&dev->lock);
...
{	
	...
	if ((hts221_get_cal_data(dev, HTS221_SENSOR_T) < 0) ||
		...
		printk(KERN_ALERT"get calibration data failed~~~~~~~ my dbg, %s %d\n", __FUNCTION__,__LINE__);
		goto input_cleanup;
	}
